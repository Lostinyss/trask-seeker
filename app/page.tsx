...
export default function Home(){
  ...
  const [viewer,setViewer]=useState<{name:string;image:string;count:number;status?:Camera["scanStatus"];message?:string}|null>(null);
  ...
  const detectCamera=async(c:Camera):Promise<Camera>=>{const isImageUrl=c.source.startsWith("http");const isPastedImage=c.source.startsWith("data:image/");if(!isImageUrl&&!isPastedImage)return{...c,status:"offline",scanStatus:"error",message:"ข้อผิดพลาด: RTSP ต้องเชื่อมผ่าน Backend Proxy"};try{let response:Response;if(isPastedImage){const blob=await (await fetch(c.source)).blob();const form=new FormData();form.append("file",blob,"clipboard-image.png");response=await fetch(`${API_BASE_URL}/detect`,{method:"POST",body:form})}else{response=await fetch(`${API_BASE_URL}/detect-url`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image_url:c.source})})}if(!response.ok){let detail="";try{detail=(await response.json()).detail||""}catch{}throw new Error(detail||`API ตอบกลับ ${response.status}`)}const data=await response.json();const count=Number(data.detection_count||0);return{...c,waste:count,annotated:data.image,status:"online",scanStatus:count>0?"detected":"clear",message:count>0?`พบขยะ ${count} จุด`:"ไม่พบขยะ"}}catch(error){const live=typeof window!=="undefined"&&!(["localhost","127.0.0.1"].includes(window.location.hostname));return{...c,annotated:c.source,status:"offline",scanStatus:"error",message:live?"แสดงภาพต้นฉบับได้ แต่ TrashTrack AI API ยังไม่ได้ Host ออนไลน์":`ข้อผิดพลาด: ${error instanceof Error?error.message:"ไม่สามารถเข้าถึง URL ได้"}`}}};
  const scanCamera=async(c:Camera)=>{setScanningCamera(c.id);if(current)update({...current,cameras:current.cameras.map(x=>x.id===c.id?{...x,scanStatus:"scanning",message:"กำลังสแกน..."}:x)});const scanned=await detectCamera(c);setPlaces(v=>v.map(p=>p.id===selected?{...p,cameras:p.cameras.map(x=>x.id===c.id?scanned:x)}:p));if(scanned.annotated)setViewer({name:scanned.name,image:scanned.annotated,count:scanned.waste,status:scanned.scanStatus,message:scanned.message});setScanningCamera(null)};
  ...
  const pasteCameraSource=(e:ClipboardEvent<HTMLInputElement>)=>{if(modal!=="camera"&&modal!=="camera-edit")return;const imageItem=Array.from(e.clipboardData.items).find(item=>item.type.startsWith("image/"));if(imageItem){e.preventDefault();const file=imageItem.getAsFile();if(!file)return;const reader=new FileReader();reader.onload=()=>setField2(String(reader.result||""));reader.readAsDataURL(file);return}const text=e.clipboardData.getData("text").trim();if(text){e.preventDefault();setField2(text)}};
  ...
  return <main className="app">
    ...
    {viewer&&<div className="modal-backdrop" role="presentation" onMouseDown={()=>setViewer(null)}><section className="camera-viewer" onMouseDown={e=>e.stopPropagation()}><button className="viewer-close" onClick={()=>setViewer(null)} aria-label="ปิด">×</button><div><p className="eyebrow">{viewer.status==="error"?"ภาพต้นฉบับ":"ผลการตรวจจับ"}</p><h2>{viewer.name}</h2></div><img src={viewer.image} alt={`ภาพจาก ${viewer.name}`}/><strong className={viewer.status==="error"?"viewer-error":viewer.count>0?"viewer-detected":"viewer-clear"}>{viewer.status==="error"?viewer.message:viewer.count>0?`พบขยะ ${viewer.count} จุด`:"ไม่พบขยะ"}</strong></section></div>}
    {modal&&<div className="modal-backdrop" role="presentation" onMouseDown={()=>setModal(null)}><form className="modal" onSubmit={e=>{e.preventDefault();submitModal()}} onMouseDown={e=>e.stopPropagation()}><div><p className="eyebrow">TRASHSEEKER</p><button type="button" onClick={()=>setModal(null)} aria-label="ปิด">×</button></div><h2>{modal==="place"?"เพิ่ม Place":modal==="edit"?"แก้ไข Place":modal==="camera"?"เพิ่ม Camera":modal==="camera-edit"?"แก้ไข Camera":"เชิญ Member"}</h2><label>{modal==="invite"?"Email":"ชื่อ"}<input autoFocus value={field1} onChange={e=>setField1(e.target.value)} required/></label>{modal!=="invite"&&<label>{modal==="camera"||modal==="camera-edit"?"Image URL, RTSP URL หรือรูปจาก Clipboard":"ที่อยู่"}<input value={field2.startsWith("data:image/")?"วางรูปภาพจาก Clipboard แล้ว":field2} onChange={e=>setField2(e.target.value)} onPaste={pasteCameraSource} placeholder={modal==="camera"||modal==="camera-edit"?"วาง URL หรือกด Ctrl + V เพื่อวางรูป":""} required={modal==="camera"||modal==="camera-edit"}/>{(modal==="camera"||modal==="camera-edit")&&<small className="paste-hint">รองรับ Ctrl + V ทั้ง URL และไฟล์รูปภาพจาก Clipboard</small>}</label>}<div className="modal-actions"><button type="button" onClick={()=>setModal(null)}>ยกเลิก</button><button className="primary" type="submit">บันทึก</button></div></form></div>}
  </main>
}
